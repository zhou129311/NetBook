...
public class ZhuiShuSQApi {
    ...
    /**
     * 获取女生区帖子列表
     * 全部、默认排序  http://api.zhuishushenqi.com/post/by-block?block=girl&duration=all&sort=updated&start=0&limit=20
     * 精品、默认排序  http://api.zhuishushenqi.com/post/by-block?block=ramble&duration=all&sort=updated&start=0&limit=20&distillate=true
     *
     * @param sort       updated(默认排序)
     *                   created(最新发布)
     *                   comment-count(最多评论)
     ...
     * @return DiscussionList
     */
    public static Entities.DiscussionList getGirlBookDisscussionList(String sort, int start, int limit, boolean distillate) {
        HttpRequest request = new HttpRequest("/post/by-block");
        HashMap<String, String> params = new HashMap<>();
        params.put("duration", "all");
        params.put("block", "girl");
        params.put("sort", sort);
        params.put("start", String.valueOf(start));
        params.put("limit", String.valueOf(limit));
        if (distillate) {

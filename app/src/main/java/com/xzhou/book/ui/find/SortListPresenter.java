...
public class SortListPresenter extends BasePresenter<SortListContract.View> implements SortListContract.Presenter {
                ...
                public void run() {
                    if (mSource == SOURCE_RANK) {
                        ...
                        if (categoryList != null) {
                            mList = new ArrayList<>();
                            parseCategoryList(categoryList.male, AppUtils.getString(R.string.male));
                            parseCategoryList(categoryList.female, AppUtils.getString(R.string.female));
    ...
    private void parseCategoryList(List<Entities.CategoryList.Category> list, String title) {
        ...
        for (Entities.CategoryList.Category category : list) {
            mList.add(new Entities.CategoryLv1(category));
        }
    }

    private void parseRankingList(List<Entities.RankingList.Ranking> list, String name) {
        if (list == null || list.size() < 1) {
        ...
        for (Entities.RankingList.Ranking ranking : list) {
            if (ranking.collapse) {
                rankLv2s.add(new Entities.RankLv2(ranking));
            } else {
                Entities.RankLv1 lv1 = new Entities.RankLv1(ranking);
                mList.add(lv1);
            }
        }
        if (rankLv2s.size() > 0) {

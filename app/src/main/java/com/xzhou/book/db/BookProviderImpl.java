...
//    }
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues values) {
        ...
        if (rowId > 0) {
            List<BookProvider.LocalBook> list = loadListInternal(db);
            if (list != null) {
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (count > 0) {
            BookManager.get().onUpdate(loadListInternal(db));
            mContext.getContentResolver().notifyChange(uri, null);
            return count;
        }
        return 0;
    ...
    public static List<BookProvider.LocalBook> loadListInternal(SQLiteDatabase db) {
        Cursor cursor = null;
        try {
            SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
            qb.setTables(TABLE_BOOK);
            String sortOrder = BookProvider.getOrder();
            cursor = qb.query(db, null, null, null, null, null, sortOrder);
